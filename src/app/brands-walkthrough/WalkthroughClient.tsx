...
export default function WalkthroughClient() {
  ...
  // Wait until the video has actually loaded enough data to scrub.
  // Safari is the strict one here:
  //   - `loadedmetadata` (readyState 1) is NOT enough — Safari refuses
  //     setCurrentTime() until readyState >= 2 (HAVE_CURRENT_DATA).
  //   - Safari may ignore preload="auto" and wait for a user gesture,
  //     so we explicitly call .load() on mount to kick off buffering.
  //   - We must NOT call .pause() before .load() — pausing a video that
  //     hasn't been told to load yet can leave it in a stuck state on
  //     Safari where seeks silently fail forever.
  ...
  useEffect(() => {
    ...
    if (v.readyState >= 2) {
      ...
      // Force Safari to start loading even if it would otherwise wait
      try {
        v.load();
      } catch {
        // load() can throw on some browsers if called too early — safe to ignore
      }
    }
  ...
  return (
    <main className="relative">
      {/* Page background — rendered as a position:fixed layer so it doesn't
          move on scroll, without using background-attachment:fixed (which
          Safari renders at low res and heavily blurs). This div sits BEHIND
          all content but stays pinned to the viewport. */}
      <div
        aria-hidden
      ...
      >
        <div className="sticky top-0 h-screen w-full flex items-center justify-center px-4 sm:px-8 lg:px-12 py-4 sm:py-6">
          ...
          <video
            ...
            muted
            playsInline
            // @ts-expect-error — legacy iOS attribute, lowercase form needed
            webkit-playsinline="true"
            // @ts-expect-error — in-app browser hint (WeChat/QQ on iOS)
            x5-playsinline="true"
            preload="auto"
            controls={false}
          ...
          >
            {/* Source children give Safari a better hint than `src=` attr —
                it can pick the right codec and start range-loading sooner. */}
            <source src="/arta-brands-office.mp4" type="video/mp4" />
          </video>
